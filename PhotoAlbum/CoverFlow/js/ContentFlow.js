...
var ContentFlowGlobal = {
    Flows: new Array,
    AddOns: {}, 
    scriptName: 'ContentFlow.js',   //Donna
    scriptElement:  null,
    ...
    initPath: function () {
        ...
        if (!this.scriptElement) {
            this.scriptName = 'ContentFlow.js'; //Donna
            this.scriptElement = this.getScriptElement(this.scriptName);
        }
 ...
 * ============================================================
 */
function ContentFlowItem(CFobj, element, index) {
    this.CFobj = CFobj;
 ...
 * ============================================================
 */
function ContentFlow(container, config) {
    if (container) {
        //Donna - Removed this as it causes a memory leak. Gadget does not need to show multiple photo albums anyhow.
        //ContentFlowGlobal.Flows.push(this);
        ContentFlowGlobal.Flows[0] = this;
    ...
    if (!this.isInit) this._init(); //Donna
};
...
ContentFlow.prototype = {
    ...
    _defaultConf: { 
        ...
        onMoveTo: function(item) {
            var coverFlow = window.coverFlow;
            
            //Fix this to fire a callback.
            if (coverFlow.duration > 0 && !coverFlow.autoAdvance) {
                var self = this;
                
                clearTimeout(coverFlow.advancePhotos);
                clearTimeout(coverFlow.interactivityTimer);
                
                //Move to next photo and restart auto-advance timer.
                coverFlow.interactivityTimer = setTimeout(function() {
                    coverFlow.autoAdvance = true;
                    self.moveTo(self.getActiveItem().index + 1);
                    coverFlow.resumeTimer();
                }, coverFlow.interactivityTimeout * 1000);
            }
            
             coverFlow.autoAdvance = false;
        },
        ...
        calcStepWidth: function(diff) {
            var vI = this.conf.visibleItems;    //3, 2
...
*/
    ...
    getItemSrc: function(index) {
        return this.items[this._checkIndex(Math.round(index))].content.getAttribute('src');
    },
     ...
     */
    _addItemCue: [],
    _addItemCueProcess: function (deleteFirst) {    //Called when items are first added.
        ...
        if (c.length > 0 && ! c[0].p) {
            ...
            window.setTimeout(function () { self._addItem(c[0].el, c[0].i, c[0].onItemAdded)}, t) ;   //Donna - added callback.
        }
    },
    addItem: function(el, index, callback) {    //Donna - added callback parameter
        this._addItemCue.push({ el: el, i: index, p: false, onItemAdded: callback});    //Donna - added callback.
        
        if (this._addItemCue.length == 1) 
            this._addItemCueProcess();
    },

    _addItem: function(el, index, onItemAdded) {
        ...
        $(".slider").css("visibility", "hidden");   //Donna - Hide scrollbar indicator when adding items.
        ...
        this.items.splice(index,0, item);   //Add item at position = index.
        ...
        window.setTimeout(function () {
            ...
            //Fire callback when item has been added.
            onItemAdded();  //Donna
        }, 100);
    ...
    _initSizes: function (x) {
        ...
        this.Flow.dimensions = this.Flow.getDimensions();   //clientWidth, clientHeight
        ...
        if (!this.Browser.IE6) {
            if (this.conf.verticalFlow && this.Container.clientWidth < this.Flow.dimensions.width) {
                ...
                //But don't move outside of parent container.
                //this.Container.style.height = this.Flow.dimensions.height+"px";   //Donna - Don't make Container larger than the Placeholder it resides in.
                //this.Flow.style.top = -(this.Flow.clientHeight - this.Container.clientHeight)+"px";    //Donna - Move top instead? Took this out because I restricted maxItemHeight instead in CoverFlow.js.
            }
        }
    ...
    _initMaxHeight: function () {
        ...
        if (this.conf.verticalFlow) {
            ...
            var proportion = screen.height/screen.width;    //screen resolution
    ...
    _step: function () {
        ...
        if ( absDiff > 0.001) { // till activeItem is nearly at position 0
            ...
            if (this.onMoved) {
                this.onMoved(window.coverFlow); //Donna - must be more elegant solution than this.
                this.onMoved = null;
            }
            //window.coverFlow.onMoved(); 
        } else {
            this._stepLock = false;
        }
...
/* ------------------------------------------------------------------------------------------------------ */
    ...
    _positionItem: function (item, relativeIndex) {
        ...
        if (item.content.origProportion) size = this._scaleImageSize(item, size);   //same as from calcSize
        item.size = size;
        ...
        els.left = (vF ? pY : pX)+"px";     //pX
        els.top = (vF ? pX : pY)+"px";  //pY
    ...
    _scaleImageSize: function (item, size, max) {   //size is result from calcSize - factor to multiple images by in order to scale?
