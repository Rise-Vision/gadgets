...
RiseVision.RSS.Settings.prototype.getSettings=function(){var a=!1,b=document.getElementsByClassName("errors")[0],c="",c=null;$(".errors").empty();$(".errors").css({display:"none"});if(""!=$("#url").val){a=rssSettings.settings.validateRequired($("#url"),b,"RSS URL")?!0:a;a=rssSettings.settings.validateRequired($("#queue"),b,"Queue")?!0:a;a=rssSettings.settings.validateNumeric($("#queue"),b,"Queue")?!0:a;0>=parseInt($("#queue").val())&&(b.innerHTML+="Queue must be greater than 0.<br />",a=!0);a=rssSettings.settings.validateRequired($("#itemsCount"),
b,"Items Count")?!0:a;a=rssSettings.settings.validateNumeric($("#itemsCount"),b,"Items Count")?!0:a;$("#itemsCount").is(":visible")&&0>=parseInt($("#itemsCount").val())&&(b.innerHTML+="Items Count must be greater than 0.<br />",a=!0);$("#separatorSize").is(":visible")&&(a=rssSettings.settings.validateNumeric($("#separatorSize"),b,"Separator Size")?!0:a);a=rssSettings.settings.validateRequired($("#transitionHold"),b,"Transition Hold")?!0:a;a=rssSettings.settings.validateNumeric($("#transitionHold"),
b,"Transition Hold")?!0:a;$("#transitionHold").is(":visible")&&0>=parseInt($("#transitionHold").val())&&(b.innerHTML+="Transition Hold must be greater than 0.<br />",a=!0);a=rssSettings.settings.validateRequired($("#transitionResumes"),b,"Transition Resumes")?!0:a;a=rssSettings.settings.validateNumeric($("#transitionResumes"),b,"Transition Resumes")?!0:a;$("#transitionResumes").is(":visible")&&0>=parseInt($("#transitionResumes").val())&&(b.innerHTML+="Transition Resumes must be greater than 0.<br />",
a=!0);a=rssSettings.settings.validateNumeric($("#headlinePadding"),b,"Headline Padding")?!0:a;a=rssSettings.settings.validateNumeric($("#storyPadding"),b,"Summary / Story Padding")?!0:a;a=rssSettings.settings.validateNumeric($("#mediaPadding"),b,"Media Padding")?!0:a;a=rssSettings.settings.validateNumeric($("#authorPadding"),b,"Author Padding")?!0:a;a=rssSettings.settings.validateNumeric($("#datePadding"),b,"Date Padding")?!0:a;"custom"==$("input[type='radio'][name='layout']:checked").val()&&(a=rssSettings.settings.validateRequired($("#layoutURL"),
b,"Layout URL")?!0:a);if(a)return $(".errors").fadeIn(200).css("display","inline-block"),$("#wrapper").scrollTop(0),null;c="up_url="+escape($("#url").val())+"&up_queue="+$("#queue").val()+"&up_itemsCount="+$("#itemsCount").val()+"&up_transition="+$("#transition").val()+"&up_scrollDirection="+$("#scrollDirection").val()+"&up_scrollSpeed="+$("#scrollSpeed").val();c+="&up_separatorColor="+$("#separatorColor").val()+"&up_separatorSize="+$("#separatorSize").val()+"&up_transitionHold="+$("#transitionHold").val()+
...
$("#layoutURL").val();c={params:c,additionalParams:JSON.stringify(rssSettings.saveAdditionalParams())};gadgets.rpc.call("","rscmd_saveSettings",null,c)}};
